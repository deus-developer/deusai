...
from decorators import command_handler, permissions, send_action
from decorators.permissions import is_admin
from decorators.users import get_users_from_text, re_id, re_username
from models import TelegramChat, Group, Player, TelegramUser, GroupPlayerThrough, Settings
from modules import BasicModule
from utils.functions import CustomInnerFilters, get_link, user_id_encode
...
class EchoModule(BasicModule):
    ...
    def __init__(self, event_manager: EventManager, message_manager: MessageManager, dispatcher: Dispatcher):
        self.add_inner_handler(InnerHandler(CommandFilter('echo'), self._echo,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
        self.add_inner_handler(InnerHandler(CommandFilter('echo_n'), self._echo_new,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
        self.add_inner_handler(InnerHandler(CommandFilter('pin'), self._pin,
    ...
    @command_handler(regexp=re.compile(r'(?P<group_name>.+)\s+\$\s+(?P<title>.+)\s*'),
    ...
    @permissions(is_admin)
    @send_action(ChatAction.TYPING)
    @command_handler(regexp=re.compile(r'(?P<recipients>.+)'),
    ...
    def _echo_new(self, update: Update, match, *args, **kwargs): #TODO: Убрать пометку сообщений, сделать её в самом методе ".message_manager.send_message"
        ...
        if group: #Если группа
            ...
            if not (ids or usernames):
                ...
                text.pop(0)
                text = '\n'.join(text)
                self.message_manager.send_message(chat_id=chat.chat_id,
        ...
        if block_list:
            ...
            self.message_manager.send_split(    chat_id=settings.GOAT_ADMIN_CHAT_ID,
                                                msg=f'❌Меня заблокировали эти игроки❌:\n{block_list}',
                                                n=30)
        if mute_list:
            mute_list = '\n'.join([f'<a href="tg://user?id={obj[1]}">{obj[0]}</a>' for obj in mute_list])
            self.message_manager.send_split(    chat_id=message.chat_id,

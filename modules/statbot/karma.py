...
from models import Player, KarmaTransition, Group, RaidAssign, RaidResult, RaidsInterval, GroupPlayerThrough
from models.raid_assign import RaidStatus
from modules import BasicModule
from decorators import command_handler, permissions
from decorators.permissions import is_admin, is_rank, or_, self_, is_developer
from decorators.users import get_players, get_player
from modules.statbot.parser import GroupParseResult
from utils.functions import CustomInnerFilters, get_link, _loose_image, \
    round_b, last_raid
...
class KarmaModule(BasicModule):
    ...
    def __init__(self, event_manager: EventManager, message_manager: MessageManager, dispatcher: Dispatcher):
        ...
        self.add_inner_handler(InnerHandler(CommandFilter('karma_add'), self._karma_add,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
        self.add_inner_handler(InnerHandler(CommandFilter('karma_reduce'), self._karma_reduce,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
        self.add_inner_handler(InnerHandler(CommandFilter('sarr'), self._sarr,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
        self.add_inner_handler(InnerHandler(CommandFilter('sakd'), self._sakd,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
        self.add_inner_handler(InnerHandler(CommandFilter('salr'), self._loose_stat_c,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
    ...
    def _karma(self, update: Update):
        ...
        self._karma_handler(module_name=karma_.module_name, sender=karma_.sender, recivier=karma_.recivier, amount=karma_.amount,
    ...
    def _karma_handler(self, module_name: str, sender: Player, recivier: Player, amount: int, description: str):
        ...
        if recivier:
            ...
            recivier.add_stats(karma=recivier.karma+amount,
                               hp=recivier.hp,
    ...
    def _taking_success(self, update: Update):
        taking_ = update.taking_success
        players = [Player.get_by_nickname(nick) for nick in taking_.players]
        for pl in players:
            if not pl:
                continue
            self._karma_handler(module_name='taking', sender=pl, recivier=pl, amount=5,
                                description=f'Захват данжа "{taking_.name}" бандой "{taking_.gang}"')
            self.message_manager.send_message(chat_id=pl.telegram_user.chat_id,
                                              text=f'Молодец бандит! С твоей помощью твоя банда {taking_.gang} смогла захватить {taking_.name}[{taking_.max_gansters}].')
    ...
    def _auto_raid_rewards_n(self):
        ...
        for result in _raid_results:
            ...
            kms.update({result['km']: result['wingoat']})
            raid_results.update({result['date']: kms})
        ...
        q = RaidAssign.update({RaidAssign.is_reported: True})\
        ...
        for raid_stats in cz_raids_q:
            ...
            player_raid_stats.update({raid_stats['player']: pl_stats})
    ...
    @command_handler(regexp=re.compile(r'\s*(?P<amount>\d+).*'),

...
from models import TelegramChat, Group
...
class ActivatedModule(BasicModule):
    ...
    def __init__(self, event_manager: EventManager, message_manager: MessageManager, dispatcher: Dispatcher):
        self.add_inner_handler(InnerHandler(CommandFilter(command='active_c', description='Активировать чат'), self._active_c,
                                            [CustomInnerFilters.from_player]))
        self.add_inner_handler(InnerHandler(CommandFilter(command='active_u', description='Активировать юзера'), self._active_u,
                                            [CustomInnerFilters.from_player]))
        self.add_inner_handler(InnerHandler(CommandFilter(command='active_g', description='Активировать группу'), self._active_g,
                                            [CustomInnerFilters.from_player]))
        super().__init__(event_manager, message_manager, dispatcher)

    @permissions(is_admin)
    @command_handler(regexp=re.compile(r'(?P<group_name>.*)'),
    ...
    def _active_g(self, update: Update, match, *args, **kwargs):
        ...
        status_n = False if group.is_active else True
        group.is_active = status_n
        group.save()


        sex_state = 'а' if update.player.settings.sex == 1 else ''
        notify_state = f'активировал{sex_state}' if status_n else f'деактивировал{sex_state}'
        notify_text = (
        ...
        self.message_manager.send_message(chat_id=update.telegram_update.message.chat_id,
                                          text=f'Группа "{group_name}" {"активирована" if status_n else "деактивированна"}')
        
    @permissions(is_admin)
    @command_handler(regexp=re.compile(r'(?P<alias>\w+)(\s+-\s+)?(?P<chat_id>.+)?'),
    ...
    def _active_c(self, update: Update, match, *args, **kwargs):
        ...
        if  chat == None or chat.chat_type == 'private':
            self.message_manager.send_message(chat_id=update.telegram_update.message.chat_id,
                                          text=f'Чата с user_id:{chat_id} не найден')
            return
        new_active = True if chat.is_active == False else False
        chat.is_active = new_active
        ...
        sex_state = 'а' if update.player.settings.sex == 1 else ''
        notify_state = f'активировал{sex_state}' if new_active else f'деактивировал{sex_state}'
        notify_text = (
                        f'[{mention_html(update.player.telegram_user_id, update.player.nickname)}]'
        ...
        self.message_manager.send_message(chat_id=update.telegram_update.message.chat_id,
                                          text=f"Чат с user_id: {chat_id} {'активирован' if new_active else 'деактивирован'}")
    ...
    def _active_u(self, update: Update, players, *args, **kwargs): #TODO: Оптимизировать проставление статусов
        ...
        for player in players:
            na = False if player.is_active == True else True
            player.is_active = na
            if na:
            ...
            notify_state = f'активировал{sex_state}' if na else f'деактивировал{sex_state}'
            notify_text = (
                            f'[{mention_html(update.player.telegram_user_id, update.player.nickname)}]'

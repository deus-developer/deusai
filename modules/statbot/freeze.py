...
from models import Player, TelegramUser, Group, GroupPlayerThrough
...
class FreezeModule(BasicModule):
    ...
    def __init__(self, event_manager: EventManager, message_manager: MessageManager, dispatcher: Dispatcher):
        self.add_inner_handler(InnerHandler(CommandFilter('freeze'), self._freeze,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
        self.add_inner_handler(InnerHandler(CommandFilter('unfreeze'), self._unfreeze,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
        self.add_inner_handler(InnerHandler(CommandFilter('unfreeze_a'), self._auto_unfreeze,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_active_chat]))
        self.add_inner_handler(InnerHandler(CommandFilter('freeze_ls'), self._freeze_ls,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
    ...
    @command_handler(regexp=re.compile(r'\s*(?P<days>\d+).*'),
    ...
    def _auto_unfreeze(self, *args, **kwargs): #TODO: Оптимизировать процесс работы с базой
        ...
        for pl in frozen_players:
            ...
            pl.frozendate = last
            pl.save()
            capture_message(f'Отпуск окончился у игрока {pl.nickname}')
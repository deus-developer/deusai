...
from models import Group, Player, Settings, GroupPlayerThrough
from modules import BasicModule
from modules.statbot.parser import GroupParseResult
from utils import format_number, format_number_padded
...
class GroupModule(BasicModule):
    ...
    def __init__(self, event_manager: EventManager, message_manager: MessageManager, dispatcher: Dispatcher):
        ...
        self.add_inner_handler(
            InnerHandler(
                CommandFilter('group_delete_with_players'),
                self.group_delete(force=True),
                [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]
            )
        )
        self.add_inner_handler(
            InnerHandler(
                CommandFilter('group_add'),
                self.group_add,
                [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]
            )
        )
    ...
    def _convene(self, update: InnerUpdate):
        if update.chat is None:
            return self.message_manager.send_message(
                chat_id=update.effective_chat_id,
                text='Эту команду можно вызвать только в группе'
            )
        ...
        if update.telegram_update.message.reply_to_message:
            reply_to_message_id = update.telegram_update.message.reply_to_message.message_id
        else:
            reply_to_message_id = None

        ping = [f'Ваш лидер {update.player.nickname} созывает вас:']
        for idx, player in enumerate(players, 1):
            ping.append(f'{idx}. {player.mention_html()}')
        ...
        self.message_manager.bot.delete_message(
            chat_id=update.effective_chat_id,
            message_id=update.telegram_update.message.message_id
        )
    ...
    def goat_handler(self, update: GroupParseResult):
        ...
        Group.insert(gangs).on_conflict(
    ...
    def group_delete(self, force: bool = False):
        @permissions(or_(is_admin, is_lider))
    ...
    def _group_unique_gansters(
        ...
        subquery = (
            ...
            .where(
                ...
                (Player.id << gangster_ids)
            )
        )

        query = (
            GroupPlayerThrough.delete()
            .where(GroupPlayerThrough.id << subquery)
        )
    ...
    def gang_handler(self, update: GroupParseResult) -> None:
        ...
        message = update.telegram_update.message
        gang = self.__handle_group(
            update.gang.leader_nickname,
            update.gang.name,
            'gang',
        ...
        goat = self.__handle_group(
            None,
            update.gang.goat_name,
            'goat',
            message.forward_date
        )

        gangster_nicknames: List[str] = [
            gangster.nickname
    ...
    def group_ls(self, show_all_groups: bool = False):
        def friendly_players_handler(
            ...
            text.append(
                f'\n<b>Итого:</b> 🏵 <code>{total_dzens}</code> '
                f'🎓<code>{format_number(total_combat_power)}</code>'
            )
            self.message_manager.send_message(
                chat_id=chat_id,
                text='\n'.join(text)
        ...
        def handler(self, update: InnerUpdate):
            ...
            for player in group.members.order_by(
                Player.dzen.desc(),

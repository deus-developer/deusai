...
class AdminModule(BasicModule):
    ...
    def __init__(self, event_manager: EventManager, message_manager: MessageManager, dispatcher: Dispatcher):
        self.add_inner_handler(InnerHandler(CommandFilter(command='ban', description='Забанить игрока'), self._ban(True),
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_active_chat]))
        self.add_inner_handler(InnerHandler(CommandFilter(command='unban', description='Разбанить игрока'), self._ban(False),
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_active_chat]))
        self.add_inner_handler(InnerHandler(CommandFilter(command='ban_ls', description='Список забанненых'), self._ban_ls,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_active_chat]))

        self.add_inner_handler(InnerHandler(CommandFilter('rp_21'), self._remove_player,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_active_chat]))

        self.add_inner_handler(InnerHandler(CommandFilter('admin_add'), self._admin(True),
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
        self.add_inner_handler(InnerHandler(CommandFilter('admin_remove'), self._admin(False),
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))
        self.add_inner_handler(InnerHandler(CommandFilter('admin_ls'), self._admin_list,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_active_chat]))
        self.add_inner_handler(InnerHandler(CommandFilter(command='crpt', description='Проверить сообщение на подпись'), self._crpt,
                                            [CustomInnerFilters.from_player, CustomInnerFilters.from_admin_chat_or_private]))

        # self.add_inner_handler(InnerHandler(CommandFilter('reverse_reduce_week'), self._reverse_reduce,
        #                                     [CustomInnerFilters.from_player, CustomInnerFilters.private]))
        super().__init__(event_manager, message_manager, dispatcher)

    @permissions(is_admin)
    @command_handler(regexp=re.compile(r'[\s\S]*﻿(?P<secret_code>.+)﻿[\s\S]*'),
    ...
    def _crpt(self, update: Update, match, *args, **kwargs):
        ...
        if not tg_user:
            return self.message_manager.send_message(chat_id=settings.GOAT_ADMIN_CHAT_ID,
    ...
    def _reverse_reduce(self, update: Update, *args, **kwargs):
        ...
        for player in players:
            ...
            for recive in recived:
                final += recive.amount
            player.add_stats(   karma=final if final != 0 else 1,
                                hp = player.hp,
    ...
    def _remove_player(self, update: Update, players, *args, **kwargs): #TODO: Придать функционал :)
        ...
        for player in players:  # group_type: Player
            ...
            capture_message(f'Игрок ({player.nickname}) удалён. Исполнитель: #{update.invoker.user_id}')
    ...
    def _ban(self, is_banned): #TODO: Дать доступ не только админам, но и лидерам его банды/альянса
        ...
        def handler(self, update: Update, players, *args, **kwargs):
            ...
            for player in players:  # group_type: Player
                ...
                capture_message(f'Пользователь #{player.telegram_user_id} {state_text}ен. Исполнитель: #{update.invoker.user_id}')
    ...
    def _admin(self, become_admin): #TODO: Оптимизировать смену статуса администратора
        ...
        def handler(self, update: Update, players, *args, **kwargs):
            ...
            for player in players:  # group_type: Player
                ...
                user.save()
                capture_message(f'Пользователь #{user.user_id} теперь {state_text} админ. Исполнитель: #{update.invoker.user_id}')

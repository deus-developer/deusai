...
from decorators.permissions import is_admin, or_, self_, is_lider
...
from models import Group, Settings, Player, PlayerStatHistory, RaidAssign, \
    RaidStatus, RaidsInterval, TelegramUser
from modules import BasicModule
from modules.statbot.karma import Karma
from utils import format_number
from utils.functions import CustomInnerFilters, get_sex_image, price_upgrade
from wasteland_wars import constants
from wasteland_wars.schemas import Profile
...
TRANSLATE_KEYS = {'hp': 'Здоровье', 'power': 'Сила', 'accuracy': 'Меткость', 'oratory': 'Харизма',
                  'agility': 'Ловкость'}
...
class StatModule(BasicModule):
    ...
    def __init__(self, event_manager: EventManager, message_manager: MessageManager, dispatcher: Dispatcher):
        ...
        self._progress_template = Template(
            open('static/templates/progress.html', 'r', encoding='utf-8').read()
        )
    ...
    def _timezone_list(self, update: InnerUpdate):
        ...
        for idx, player in enumerate(query, 1):
            output.append(
                f'{idx}. {player.mention_html()}:'
                f'\t\t{player.settings.timedelta}⏳'
            )
    ...
    def _show_player_stats(self, player: Player, chat_id: int, editable: bool = False):
        formatted_stat = (
            f'{get_sex_image(player.settings.sex if player.settings else 0)} <b>{player.nickname}</b>\n'
        )

        def get_groups(group_type: str):
            return (
                group.name
                for group in player.members
        ...
        last_updated_at_text = dt.distance_of_time_in_words(
            stats.time if stats else player.last_update,
            to_time=time.time()
        )
        formatted_stat += (
            f'📅Обновлён <b>{last_updated_at_text}</b>\n\n'
        )
    ...
    def _show_player_cap(self, player: Player, chat_id: int, editable: bool = False):
        if editable:
            ...
            formatted_cap = (
                ...
                f'осталось до капов 🏵{player.dzen}:'
            )
    ...
    def _show_player_raid_reward(self, player: models.Player, chat_id, editable=False):
        ...
        if not goat:
            return self.message_manager.send_message(
                chat_id=chat_id,
                text=(
                    'Ты без козла => без рейдов'
                    if editable else
                    f'{player.mention_html()} без козла => без рейдов'
                )
            )
        ...
        if not league:
            return self.message_manager.send_message(
                ...
                text=(
                    ...
                    if editable else
                    f'Ля, не знаю лигу {player.mention_html()}. Кинь панель его козла.'
                )
            )
    ...
    def _raid_stat_inline(self, update: InnerUpdate):
        player_id, interval_id = [
            int(x)
            for x in self._re_raid_stat_inline.search(update.telegram_update.callback_query.data).groups()
        ]
    ...
    def _show_player_raid_stat(
        ...
        if invoker.is_admin and update.player.telegram_user_id != invoker.user_id:
            ...
            formatted_report = (
                f'<b>Рейдовая статистика📊 '
                f'{player.mention_html()}</b>\n'
            )
        else:
            name = 'Ты'
    ...
    def _raid_info_inline(self, update: InnerUpdate):
        player_id, interval_id = [
            int(x)
            for x in self._re_raid_info_inline.search(update.telegram_update.callback_query.data).groups()
        ]
    ...
    def _show_player_raids_info(
        ...
        if invoker.is_admin and invoker.user_id != player.telegram_user_id:
            formatted_report = (
                f'<b>Детальная рейдовая статистика📊 '
                f'{player.mention_html()}</b>\n'
            )
        else:
        ...
        for raid in raids['all']:
            ...
            formatted_report += (
                f'[{raid.time.strftime("%d.%m %H-%M")}]  {icon}'
                f'{raid.km_assigned:02}км  |{knuckle}\n'
            )
        ...
        buttons[0].append(
            InlineKeyboardButton(
                text='📄Кратко',
                callback_data=f'raids_{player.id}_{interval.id}'
            )
        )
        if next_interval_id:
            buttons[0].append(
    ...
    def _update_player(self, player: Player, profile: Profile, chat_id: int, created: bool = False):
        stats = profile.stats
        rewards = 0
        total = 0

        output: List[str] = [
            '<b>Награда за прокачку</b>'
        ]
        ...
        player.add_stats(
            ...
            dzen=stats.dzen,
            raids21=player.raids21,
            raid_points=player.raid_points,
            loose_raids=player.loose_raids,
            loose_weeks=player.loose_weeks,
            karma=player.karma
        )
        ...
        karma_update = InnerUpdate()
        karma_update.karma_transaction = Karma(
            module_name='stat',
            recivier=player,
            sender=player,
            amount=rewards,
            description=f'Начисление кармы за прокачку.'
        )
        self.event_manager.invoke_handler_update(karma_update)
    ...
    def _save_stats(self, update: PlayerParseResult):
        ...
        if (
            update.player is None and
            profile.telegram_user_id is None
        ):
            return self.message_manager.send_message(
                chat_id=message.chat_id,
                text='Пришли мне полный ПИП-БОЙ'
            )

        if (
            profile.telegram_user_id and
            profile.telegram_user_id != update.invoker.user_id
        ):
            return self.message_manager.send_message(
                chat_id=message.chat_id,
                text='Это не твой ПИП-БОЙ'
            )

        if (
            update.player is None and
            update.timedelta > datetime.timedelta(minutes=5)
        ):
            return self.message_manager.send_message(
                chat_id=message.chat_id,
                text='А можно ПИП-БОЙ посвежее?'
            )

        if (
            update.player and
            update.player.nickname != profile.nickname and
            update.invoker.user_id != profile.telegram_user_id
        ):
            return self.message_manager.send_message(
                chat_id=message.chat_id,
                text='Пришли мне полный ПИП-БОЙ'
        ...
        if (
            update.player and
            update.player.last_update >= update.date
        ):
            return self.message_manager.send_message(
                chat_id=message.chat_id,
        ...
        if (
            profile.telegram_user_id == update.invoker.user_id and
    ...
    @permissions(or_(is_admin, self_))
    def _karma_chart(self, update: InnerUpdate, players: List[Player]):
        players = players or ([update.player] if update.command.argument == '' else [])
        if not players:
            return

        for player in players:
            self._show_player_karma(player, update.effective_chat_id)

    def _show_player_karma(self, player: Player, chat_id: int):
        ...
        if not stats_history:
            return self.message_manager.send_message(
                chat_id=chat_id,
                text=f'Я поражён.... У игрока {player.mention_html()} '
                     f'нет истории кармы....'
            )
            ...
            dataset.append(
                {
                    ...
                    'dataset': [
                        {'unix': times[idx], 'y': value}
                        for idx, value in enumerate(stats[KEY_STAT])
                    ]
                }
            )

        text = self._progress_template.render(
            dataset=dataset,

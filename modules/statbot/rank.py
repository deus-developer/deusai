...
class RankModule(BasicModule):  # TODO: Полностью переработать
    ...
    @command_handler(
        regexp=re.compile(r'\s*\[(?P<emoji>.+)]\s*\[(?P<name>.+)]\s*\[(?P<priority>\d+)]\s*(?P<text>[\s\S]*)'),

...
class TakingModule(BasicModule):  # TODO: Доработать
    ...
    def _taking_accept(self, update: Update):
        ...
        if update.player not in taking.group.members:
            return self.message_manager.bot.answer_callback_query(
                ...
                show_alert=False,
                text='Ты не состоишь в этой ударной группе'
            )
    ...
    @command_handler(
        regexp=re.compile(r'(?P<group>.+)\s+(?P<km>\d+)'),
    ...
    def _taking_create(self, update: Update, match, *args, **kwargs):
        ...
        if taking.count() > 0:
            ...
            return self.message_manager.send_message(
                chat_id=message.chat_id, text=f'Захват "{Wasteland.take_locations_by_km.get(km, ["[подземелье]"])[0]}" уже начат группой "{group.name}".\n'

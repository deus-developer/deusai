...
class ShopModuleUpdate(BasicModule):  # TODO: Ввести механизм скидок
    ...
    def __init__(self, event_manager: EventManager, message_manager: MessageManager, dispatcher: Dispatcher):
        self.add_inner_handler(
            InnerHandler(
                CommandFilter('shop1'), self._shop_open,
                [CustomInnerFilters.from_player, CustomInnerFilters.from_active_chat]
            )
        )
        self.add_inner_handler(
            InnerHandler(
                CommandNameFilter('auctionp'), self._shop_auction_buy,
                [CustomInnerFilters.from_player, CustomInnerFilters.from_active_chat]
            )
        )
    ...
    def _shop_auction_buy(self, update: Update):
        ...
        member = member.get() if member.exists() else AuctionMember(auction=auction, player=update.player, price=0, last_update=update.date)
    ...
    def _crm_task_edit(self, purchare: ShopPurchase):
        ...
        if purchare.last_update < now - datetime.timedelta(hours=12):
            ...
            message = self.message_manager.send_message(
                chat_id=purchare.chat_id, reply_markup=markup,
            ...
            self.message_manager.update_msg(
                chat_id=purchare.chat_id, message_id=purchare.message_id,
    ...
    def _crm_refuse(self, update: Update):
        ...
        u = Update()
        u.karma_ = Karma(
            module_name='shop', recivier=purchare.player, sender=purchare.player,

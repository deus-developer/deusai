...
from models import KarmaTransition, Settings
from utils.functions import get_next_raid_date
from .base import BaseModel
from .telegram_user import TelegramUser

if TYPE_CHECKING:
    from models import RaidAssign, Group
...
class Player(BaseModel, Model):
    telegram_user = peewee.ForeignKeyField(TelegramUser, backref='player', on_delete='CASCADE', null=True)  # users.id
    ...
    def lider_access_query(self):
        from models import GroupPlayerThrough, Group
        return self.select() \
            .join(GroupPlayerThrough,
                  on=((Player.id == GroupPlayerThrough.player_id) & (
    ...
    def raid_near_time(self, time: Optional[datetime.datetime] = None) -> Optional["RaidAssign"]:
        ...
        latest_raid = self.raids_assign \
            .filter(RaidAssign.time == get_next_raid_date(time)) \
    ...
    def get_iterator(cls):
        return cls.select() \
            .join(TelegramUser) \
            .where(
            (Player.telegram_user.is_banned == False) and (Player.is_active == True))

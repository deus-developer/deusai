...
from src.decorators import permissions
from src.decorators.permissions import is_admin, or_, self_
...
class SettingsModule(BasicModule):
    ...
    def _show_player_settings(self, player: Player, chat_id: int, editable: bool):
        formatted_settings = f"\t\t\t\t<b>{player.nickname}</b>\n" f"\t\t\t\t\t<b>Настройки</b>\t\t\t\t\t\n"
        formatted_settings += f'Пинги: {editable_command("settings_pings", editable)}'

        self.message_manager.send_message(chat_id=chat_id, text=formatted_settings)

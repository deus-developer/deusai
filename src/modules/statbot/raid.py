...
from src.decorators import command_handler, permissions, get_invoker_raid
...
from src.modules.statbot.parser import PlayerParseResult, GroupParseResult
from src.utils import get_next_raid_date, get_last_raid_date, get_when_raid_text
from src.utils.functions import CustomInnerFilters
from src.wasteland_wars import constants
from src.wasteland_wars.schemas import GangMember
...
class RaidModule(BasicModule):
    ...
    def _update_from_panel_gang(self, update: GroupParseResult):
        ...
        ganster_by_nickname: Dict[str, GangMember] = {gangster.nickname: gangster for gangster in update.gang.members}
        ...
        for gangster_player in Player.select().where(Player.nickname << list(ganster_by_nickname.keys())):
            ...
            if (
                raid_assign.km_assigned == gangster.kilometr
                and gangster.status == "👊"
                and raid_assign.status not in (RaidStatus.CONFIRMED, RaidStatus.IN_PROCESS)
            ):
            ...
            elif raid_assign.status == RaidStatus.IN_PROCESS and (
                raid_assign.km_assigned != gangster.kilometr or gangster.status != "👊"
            ):
                raid_assign.status = RaidStatus.LEFTED
                self.message_manager.send_message(
                    chat_id=gangster_player.telegram_user_id,
                    text=f"Эй, стой! Куда с рейда убежал?\n"
                    f"РЕЙД НА {raid_assign.km_assigned}км. "
                    f"!!!ОДУМАЙСЯ!!!\n{raid_assign.time}",
                )
            elif (
                raid_assign.status == RaidStatus.ACCEPTED
                and raid_assign.km_assigned >= gangster.kilometr
                and gangster.status == "👟"
            ):
                raid_assign.status = RaidStatus.ACCEPTED
    ...
    def _raid_status_update(self, update: InnerUpdate, match, players):
        ...
        query = RaidAssign.update({RaidAssign.status_id: status_id, RaidAssign.is_reported: False}).where(
    ...
    def _assign_players(self, update: InnerUpdate, match, players):
        ...
        if len(update.player.liders) == 0:
            return self.message_manager.send_message(chat_id=update.invoker.chat_id, text="Нет доступа!")
        ...
        if not km.isdigit():
            return self.message_manager.send_message(
                chat_id=chat_id, text=f"Пришли км, на который отправить группу, числом"
            )
        else:
            km = int(km)

        if km not in constants.raid_kms:
            return self.message_manager.send_message(chat_id=chat_id, text="Такой рейдовой локации не существует")
        ...
        if error_groups:
            self.message_manager.send_message(chat_id=chat_id, text=f'Я не знаю групп: {"; ".join(error_groups)}')

        if groups or pls:
            self.message_manager.send_message(
                chat_id=chat_id,
                text=f'{", ".join(groups + pls)} в ' f'{next_raid_time.time()} рейдят {km}км',
            )
        else:
            self.message_manager.send_message(chat_id=chat_id, text=f"Кажется ты никого не послал на рейд.")
    ...
    def _sendpin(self, update: InnerUpdate):
        ...
        if update.command.argument == "":
            ...
            for group_name in group_names:
                group = Group.get_by_name(group_name)
                if group is None or group not in update.player.liders:
                    continue
            ...
            for km in kms:
                for pin in RaidAssign.next_raid_players(status=RaidStatus.UNKNOWN, km=km):
                    player = pin.player
                    if player in players or player not in access:
                        continue
                    players.append(player)
        ...
        if not mentions:
            return self.message_manager.send_message(
                chat_id=chat_id, text="Пины еще не назначены или уже все отправлены"
            )
    ...
    def _my_raidpin(self, update: InnerUpdate, raid_assigned):
        ...
        self.message_manager.send_message(chat_id=invoker.chat_id, text=raid_assigned.get_msg())
    ...
    def _raidpin_accept(self, update: InnerUpdate, raid_assigned):
        ...
        if raid_assigned.status == RaidStatus.HASNOTSEEN:
            return self.message_manager.send_message(
                chat_id=invoker.chat_id, text=self.get_assigned_message(raid_assigned)
            )
        elif raid_assigned.status >= RaidStatus.ACCEPTED:
            return self.message_manager.send_message(chat_id=invoker.chat_id, text="Ты уже принял назначение на рейд")
    ...
    def _raidpin_reject(self, update: InnerUpdate, raid_assigned):
        ...
        if raid_assigned.status == RaidStatus.HASNOTSEEN:
            ...
            return self.message_manager.send_message(chat_id=invoker.chat_id, text="Ты уже отказался от рейда")
        ...
        self.message_manager.send_message(
            chat_id=settings.GOAT_ADMIN_CHAT_ID,
            text=f"Игрок {update.player.mention_html()} отказался " f"от пина [{raid_assigned.km_assigned}]",
        )
    ...
    def _get_players_km_new(self, update: InnerUpdate, players: List[Player]):
        ...
        radar_query_dates = (
            Radar.select(Radar.player_id, peewee.fn.MAX(Radar.time).alias("MAXDATE"))
            .group_by(Radar.player_id)
            .alias("radar_max")
        )
        ...
        status_case = peewee.Case(
            ...
            (
                (
                    RaidAssign.status_id << [RaidStatus.IN_PROCESS, RaidStatus.CONFIRMED],
                    0,
                ),
        ...
        _players = (
            ...
            .distinct()
            .order_by(
                RaidAssign.km_assigned.desc(),
                status_case.asc(),
                radar_query.c.km.desc(),
                Player.sum_stat.desc(),
                Player.dzen.desc(),
            )
        )
        ...
        for _player in _players.dicts():
            ...
            formatter_report += (
                f"{emojie}{km_assigned:02}|{emojie_speed}{km_radar:02}|{delta}|🎓{sum_stat:03.1f}|"
        ...
        def handler(self, update: InnerUpdate):
            ...
            def format_line(raid_status):
                if raid_power[raid_status] > 0:
                    return (
                        f": <b>{raid_counts[raid_status]}</b> [{int(raid_counts[raid_status] / counts * 100)}%]"
            ...
            for arg in kms:
                ...
                self.message_manager.send_message(chat_id=message.chat_id, text=message_text)
    ...
    def _masterpin(self, show_last_raid: bool = False):
        ...
        def handler(self, update: InnerUpdate):
            ...
            def format_line(raid_status):
                return (
                    f'<b>{len(raid_users[raid_status])}</b>[{raid_power[raid_status]}]: '
                    f'{"; ".join(raid_users[raid_status])}'
                    if raid_users[raid_status]
                    else ""
                )
    ...
    def _update_from_profile(self, update: PlayerParseResult):
        ...
        if (
            raid_assign.status not in (RaidStatus.IN_PROCESS, RaidStatus.CONFIRMED)
            and not raid_assign.is_reported
            and raid_assign.km_assigned == update.profile.distance
        ):

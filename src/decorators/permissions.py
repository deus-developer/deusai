...
def or_(*filters):
    def calc(update: InnerUpdate, *args, **kwargs):
        return any(func(update, *args, **kwargs) for func in filters)
...
def and_(*filters):
    def calc(update: InnerUpdate, *args, **kwargs):
        return all(func(update, *args, **kwargs) for func in filters)
        ...
        def decorator(self, update: InnerUpdate, *args, **kwargs):
            ...
            if player is None:
                return self.message_manager.send_message(
                    ...
                    text="Я тебя не знаю, пришли свой полный пип-бой, " "чтобы познакомиться",
                )
            ...
            if not permit_expression(update, *args, **kwargs):
                ...
                if update.effective_chat_id != invoker.chat_id:
                    return
                return self.message_manager.send_message(chat_id=invoker.chat_id, text="Доступ запрещен")

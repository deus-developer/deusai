...
def _get_next_raid_date(date: datetime.datetime) -> datetime.datetime:
    ...
    return date + datetime.timedelta(hours=h) - datetime.timedelta(
        minutes=date.minute,
        seconds=date.second,
...
class CustomFilters:
    class WWForwarded(BaseFilter):
        ...
        def filter(self, message):
            return (
                ...
                message.forward_from.id == constants.chat_id
            )
    ...
    class GreatWar(BaseFilter):
        ...
        def filter(self, message):
            return (
                message.chat and
                message.chat.id == constants.greatwar_chat_id
            )
    ...
    class TolylyForwarded(BaseFilter):
        ...
        def filter(self, message):
            return (
                ...
                message.forward_from_chat == constants.tolyly_chat_id
            )
...
class CustomInnerFilters:
    ...
    def private(update: InnerUpdate) -> bool:
        ...
        return (
            update.telegram_update and
            update.telegram_update.message and
            update.telegram_update.message.chat.type == 'private'
        )
    ...
    def chat(update: InnerUpdate) -> bool:
        ...
        return (
            update.telegram_update and
            update.telegram_update.message and
            update.telegram_update.message.chat.type in ('group', 'supergroup')
        )
    ...
    def from_player(update: InnerUpdate) -> bool:
        ...
        return (
            update.invoker and
            not update.invoker.is_banned and

            update.player and
            update.player.is_active
        )
    ...
    def from_admin_goat_chat(update: InnerUpdate) -> bool:
        return (
            update.telegram_update and
            update.telegram_update.message and
            update.effective_chat_id == settings.GOAT_ADMIN_CHAT_ID
        )

    @classmethod
    def from_admin_chat_or_private(cls, update: InnerUpdate) -> bool:
        return (
            cls.private(update) or
            cls.from_admin_goat_chat(update)
        )
